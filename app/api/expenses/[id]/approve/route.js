...
export const PUT = withApi(async (req, { params }) => {
  ...
  if (expense.status !== 'PENDING') return fail('Only pending claims can be approved', 400)
  ...
  if (session.role === 'MANAGER') {
    ...
    if (!employee || String(employee.reportingManager) !== session.userId) {
      return fail('You can only approve claims from your own direct reports', 403)
    }
  }
  ...
  await logAction(session, {
    ...
    description: `Expense claim of ${expense.amount} approved`,
  })

  return ok(expense, 'Expense approved')
})
