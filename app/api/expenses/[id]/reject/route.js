...
export const PUT = withApi(async (req, { params }) => {
  ...
  if (expense.status !== 'PENDING') return fail('Only pending claims can be rejected', 400)
  ...
  if (session.role === 'MANAGER') {
    ...
    if (!employee || String(employee.reportingManager) !== session.userId) {
      return fail('You can only reject claims from your own direct reports', 403)
    }
  }
  ...
  await logAction(session, {
    ...
    description: `Expense claim of ${expense.amount} rejected`,
  })

  return ok(expense, 'Expense rejected')
})
