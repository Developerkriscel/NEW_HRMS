...
async function tryTenantById(email, tenantId) {
  ...
  const tenant = await Tenant.findOne({ _id: tenantId, deleted: false }).maxTimeMS(USER_LOOKUP_MAX_TIME_MS)
...
export async function findUserByEmail(email, options = {}) {
  ...
  const superAdmin = await PlatformOperator.findOne({ email: normalizedEmail }).maxTimeMS(USER_LOOKUP_MAX_TIME_MS)
  ...
  const tenants = await Tenant.find({
    deleted: false,
    status: { $ne: 'CANCELLED' },
  }).select('_id tenantCode companyName databaseName databaseStatus').maxTimeMS(USER_LOOKUP_MAX_TIME_MS)
