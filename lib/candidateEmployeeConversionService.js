...
async function loadConversionContext(tenantId, preboardingId) {
  ...
  const [tenant, candidate, application, job, offer, version, personal, emergencyContact, bank, statutory, documents] = await Promise.all([
  ...
  const [department, designation, branch, manager] = await Promise.all([
  ...
  return {
    ...
    personal, emergencyContact, bank, statutory, documents,
    department, designation, branch, manager,
  }
}
...
function buildMasterRecord(ctx, employeeCode = proposedEmployeeCode(ctx)) {
  ...
  return {
    ...
    workingHours: ctx.tenant?.hrSettings?.officeStartTime && ctx.tenant?.hrSettings?.officeEndTime
      ? `${ctx.tenant.hrSettings.officeStartTime}-${ctx.tenant.hrSettings.officeEndTime}`
      : null,
    inTiming: ctx.tenant?.hrSettings?.officeStartTime || null,
    ...
    weekOff: Array.isArray(ctx.tenant?.hrSettings?.weeklyOff) ? ctx.tenant.hrSettings.weeklyOff.join(', ') : null,
...
function checklistFor(ctx, master) {
  ...
  const preconditions = [
    ...
    { key: 'informationApproved', label: 'Information', status: ctx.preboarding.formStatus === FORM_STATUS.APPROVED ? 'Approved' : 'Pending', ready: ctx.preboarding.formStatus === FORM_STATUS.APPROVED },
    { key: 'documentsVerified', label: 'Documents', status: ctx.preboarding.verificationStatus === VERIFICATION_STATUS.COMPLETE ? 'Verified' : 'Pending', ready: ctx.preboarding.verificationStatus === VERIFICATION_STATUS.COMPLETE },
  ...
  const joined = ctx.preboarding.status === PREBOARDING_STATUS.JOINED
  const mandatoryReady = missingRequired.length === 0 && preconditions.every((row) => row.ready)
  return {
    required,
    optional,
    preconditions,
      ...
      const locked = await Preboarding.findOneAndUpdate(
        {
          ...
          conversionStatus: { $in: [EMPLOYEE_CONVERSION_STATUS.READY, EMPLOYEE_CONVERSION_STATUS.FAILED] },
        },
        {
          $set: {
