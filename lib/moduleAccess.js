...
export function hasModuleAccess(userOrSession, moduleKey) {
  if (Array.isArray(moduleKey)) return moduleKey.some((key) => hasModuleAccess(userOrSession, key))
...
export function moduleForApiPath(pathname) {
  if (pathname.startsWith('/api/recruitment/applications/')) {
    ...
    if (
      pathname.includes('/bulk-action')
      || pathname.includes('/reject')
      || pathname.includes('/talent-pool')
      || pathname.includes('/assign-recruiter')
      || pathname.includes('/notes')
      || pathname.includes('/stage-history')
    ) return [MODULE_ACCESS.PIPELINE, MODULE_ACCESS.CANDIDATES]
