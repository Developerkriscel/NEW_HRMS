...
import { isAccountSessionRevoked, touchAccountSession } from '@/lib/accountSessions'
...
function buildClaims(user) {
  if (user.isSuperAdmin) {
    return {
      ...
      accountSessionId: user.accountSessionId || null,
...
const tokenBlacklistCache = global._nexahrTokenBlacklistCache || new Map()
const sessionRevokedCache = global._nexahrSessionRevokedCache || new Map()
global._nexahrTokenBlacklistCache = tokenBlacklistCache
global._nexahrSessionRevokedCache = sessionRevokedCache

export async function blacklistToken(token) {
  const decoded = decodeJwt(token)
  if (decoded?.devLogin && process.env.NODE_ENV !== 'production') return
  tokenBlacklistCache.set(token, { expiresAt: Date.now() + 3600000, blacklisted: true })
  await connectDB()
...
export async function revokePlatformSession(sessionId, { revokedBy = null, reason = null } = {}) {
  if (!sessionId) return
  sessionRevokedCache.set(sessionId, { expiresAt: Date.now() + 3600000, revoked: true })
  await connectDB()
...
export async function isPlatformSessionRevoked(sessionId) {
  ...
  const isRevoked = !!session.revoked || session.expiresAt < new Date()
  sessionRevokedCache.set(sessionId, {
    expiresAt: Date.now() + 60000,
    revoked: isRevoked,
  })
  return isRevoked
...
export async function getSession() {
  ...
  if (!decoded.devLogin && decoded.accountSessionId) {
    if (await isAccountSessionRevoked(decoded.accountSessionId)) return null
    await touchAccountSession(decoded.accountSessionId)
  }
