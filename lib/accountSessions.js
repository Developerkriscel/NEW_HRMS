...
const sessionTouchCache = global._nexahrAccountSessionTouchCache || new Map()
global._nexahrAccountSessionRevokedCache = sessionRevokedCache
global._nexahrAccountSessionTouchCache = sessionTouchCache
...
export async function touchAccountSession(sessionId) {
  ...
  const cached = sessionTouchCache.get(cacheKey)
  if (cached && cached > Date.now()) return

  sessionTouchCache.set(cacheKey, Date.now() + 5 * 60 * 1000)
  await connectDB()
